...
const TmkParserPage = observer((props) => {
    ...
    const onClickButtonAddNewProduct = async () => {
        ...
        let mess
        if (number > 0) {
            await addProduct(props.brand, formData, number).then(data => {
                setMessage(data)
            })
        }else {
            let length = await getLength(props.brand, formData)
            let quantity = 10
            mess = "Начало:"
            for(let i = 1; i <= length; i = i + quantity) {

                await addProduct(props.brand, formData, i, quantity)
                    // eslint-disable-next-line
                    .then(data => {
                        if (data?.error) {
    ...
    let onClickButtonChangePrices = async () => {
        ...
        // }
        // setLoading(true)
        // await changePrices(props.brand, formData)
        //     // eslint-disable-next-line
        //     .then(data => {
        //         if (data?.error) {
